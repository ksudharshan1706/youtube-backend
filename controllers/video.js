...
exports.music = async (req, res, next) => {
  try {
    const videos = await Video.find({ tags: "music" }).limit(20);
    res.status(200).json(videos);
  } catch (err) {
    next(err);
  }
...
exports.sports = async (req, res, next) => {
  try {
    const videos = await Video.find({ tags: "cricket" }).limit(20);
    res.status(200).json(videos);
  } catch (err) {
    next(err);
  }
...
exports.gaming = async (req, res, next) => {
  try {
    const videos = await Video.find({
      tags: { $in: ["gamming", "gaming"] },
    }).limit(20);
    res.status(200).json(videos);
  } catch (err) {
    next(err);
